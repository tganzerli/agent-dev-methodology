...
| **MCP server catalog** (one page, mirrored client configs) | A server added for one agent was invisible to the others; the same task succeeded under one agent and failed under another for a reason invisible in the code | `core/mcp.md` (install only if the project uses MCP) |
| **Destructive-operation gate + documented-exception shape** (per-operation `[y/n]`; an exception needs authorization + allow-list + zero-change deny-list + commit prefix + provenance) | Agents reached for `--force` / `reset --hard` to reach a clean tree, and judged case by case that a commit was "trivial enough" to skip the gate | `core/rules/git_branching_rule.md` §9.1-9.2 |
| **Cross-bench synthesis in the benchmark index** | The project's real question ("what is the gain of X?") had different answers per regime, and each page answered only its own | `modules/benchmarks/` (rule §6.1) |
| **Non-terminal `future`/`paused` statuses** | A decided-but-deferred item had nowhere to live: archived as history it got re-litigated, left `open` it looked like work in flight | `core/skills/work-index` + conventions §6 |
| **Sub-trio (`parent_task`)** | The convention was in use across 8 artifacts with every parent task present, while no script read the field and no doc described it — so every sub-trio rendered as a broken wikilink with a degraded summary | `core/skills/work-index` + conventions |
| **Prose deliverables** (directory-scoped writing cycle, `⚠ source needed`, `_norms`/`sources` split, institutional-override table) | The trio cycle does not fit a chapter, so it was being bypassed inside `content/` silently — while prose is exactly where an unanchored claim leaves the repo and reaches readers who cannot check it | `modules/writing/` |
